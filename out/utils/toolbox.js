...
const Tools = {
    ...
    apiResponse(res, ResponseType, statusCode, responseCode, data, responseMessage = '', rootElement = '') {
        const status = ResponseType ? 'success' : 'failure';
        const response = {
            status,
            responseCode: ResponseType ? '00' : responseCode,
            responseMessage: responseMessage || (0, http_status_codes_1.getReasonPhrase)(statusCode),
            details: data,
        };
        (0, logger_1.default)('tracing...', responseMessage || JSON.stringify(data) || 'unknown error');
        return res.format({
            json: () => {
                res.type(_types_1.applicationJsonType);
                res.status(statusCode).send(response);
            },
            xml: () => {
                res.type(_types_1.applicationXmlType);
                res.status(statusCode).send((0, js2xmlparser_1.parse)(rootElement || 'response', response));
            },
            default: () => {
                res.status(http_status_codes_1.StatusCodes.NOT_IMPLEMENTED).send((0, http_status_codes_1.getReasonPhrase)(http_status_codes_1.StatusCodes.NOT_IMPLEMENTED));
            },
        });
    },
    hashFunc(payload = '') {
        return crypto_1.default.createHash('sha512').update(payload).digest('hex');
    },
    /**
     ...
     */
    isXDaysFromNow(date, days) {
        const today = (0, moment_1.default)();
        const dateToCheck = (0, moment_1.default)(date);
